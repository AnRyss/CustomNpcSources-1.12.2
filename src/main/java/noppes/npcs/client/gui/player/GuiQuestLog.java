...
implements ITopButtonListener, ICustomScrollListener, GuiYesNoCallback, IGuiData {
	...
	protected void actionPerformed(GuiButton guibutton) {
        // New button to on/off arrow navigation
        if (guibutton.id == 31 && this.selectedQuest != null && this.selectedQuest.questInterface != null) {
            GuiNpcButton button = (GuiNpcButton) guibutton;
            this.selectedQuest.questInterface.toRenderArrow = (button.getValue() == 1);
             NoppesUtilPlayer.sendData(EnumPlayerPacket.QuestNavigationStatus, this.selectedQuest.id, button.getValue());
        }
	...
	public void initGui() {
		...
		if (this.selectedQuest != null) {
			...
			this.yoffset -= 16;
            // New arrow switcher
            boolean qi = this.selectedQuest.questInterface != null;
            if(qi) {
                addButton(new GuiNpcButton(31, this.guiLeft + 95, this.guiTop + 170, 60, 20,
                        new String[]{"Вкл.Навигацию", "Выкл.Навигацию"},
                        selectedQuest.questInterface.toRenderArrow ? 1 : 0));
            }
		} else {
