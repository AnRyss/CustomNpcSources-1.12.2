...
public class PacketHandlerPlayer {
	...
	private void player(ByteBuf buffer, EntityPlayerMP player, EnumPlayerPacket type) throws Exception {
        ...
        else if (type == EnumPlayerPacket.QuestNavigationStatus) {
            int questId = buffer.readInt();   // ID квеста
            int value   = buffer.readInt();   // 0 или 1 — значение кнопки
            PlayerData playerData = PlayerData.get(player);
            if (playerData == null) {
                return;
            }
            PlayerQuestData questController = playerData.questData;
            QuestData questData = questController.activeQuests.get(questId);

            if (questData != null && questData.quest != null && questData.quest.questInterface != null) {

                questData.quest.questInterface.toRenderArrow = (value == 0);
