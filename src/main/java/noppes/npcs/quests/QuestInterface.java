...
public class QuestInterface {
    ...
    public boolean toRenderArrow;
    private List<QuestCordinates> navigationCoordinates = new ArrayList<>();

    // Методы для работы с координатами
    public boolean hasNavigation() {
        return !navigationCoordinates.isEmpty();
    }
    public void addNavigationCoordinate(QuestCordinates coord) {
        navigationCoordinates.add(coord);
    }

    public void removeNavigationCoordinate(int index) {
        if (index >= 0 && index < navigationCoordinates.size()) {
            navigationCoordinates.remove(index);
        }
    }

    public void clearNavigationCoordinates() {
        navigationCoordinates.clear();
    }

	public QuestInterface() {
		this.items = new NpcMiscInventory(1);
		this.tasks = new QuestObjective[0];
        this.toRenderArrow = true;
	}
    ...
    public void writeEntityToNBT(NBTTagCompound compound) {
        fix();
        compound.setBoolean("RenderArrow", toRenderArrow);
        if (!navigationCoordinates.isEmpty()) {
            NBTTagList coordsList = new NBTTagList();
            for (QuestCordinates coord : navigationCoordinates) {
                coordsList.appendTag(coord.toNBT());
            }
            compound.setTag("NavCoords", coordsList);
        }
        NBTTagList taskList = new NBTTagList();
        for (int i = 0; i < this.tasks.length; i++) {
            taskList.appendTag(this.tasks[i].getNBT());
        }
        compound.setTag("Tasks", taskList);

}
	public void readEntityFromNBT(NBTTagCompound compound) {
        this.toRenderArrow = compound.getBoolean("RenderArrow");
		this.id = compound.getInteger("Id");

        navigationCoordinates.clear();
        if (compound.hasKey("NavCoords", 9)) { // 9 = TagList
            NBTTagList list = compound.getTagList("NavCoords", 10); // 10 = Compound
            for (int i = 0; i < list.tagCount(); i++) {
                navigationCoordinates.add(QuestCordinates.fromNBT(list.getCompoundTagAt(i)));
            }
        }
    ...
    public List<QuestCordinates> getNavigationCoordinates() {
        return navigationCoordinates;
    }

    public void setNavigationCoordinates(List<QuestCordinates> navigationCoordinates) {
